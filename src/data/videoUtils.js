 ...
 */
export async function extractVideoThumbnail(videoDataURL, timeSeconds = 0.5) {
  return new Promise((resolve) => {
    ...
    video.onloadedmetadata = () => {
      ...
      // Use a very early frame to ensure it exists
      const captureTime = Math.min(0.1, video.duration * 0.05);
      ...
      setTimeout(() => {
        try {
          ...
          if (thumbnail && thumbnail.length > 1000) { // Basic validity check
            ...
            resolveWithPlaceholder('Generated thumbnail is invalid');
          }
        } catch (error) {
...
export function getVideoThumbnail(post) {
  console.log('getVideoThumbnail called with post:', { 
    hasVideoSrc: !!post.videoSrc, 
    hasVideoThumbnail: !!post.videoThumbnail,
    videoSrcType: typeof post.videoSrc,
    videoSrc: post.videoSrc?.substring(0, 50) + '...' // Show first 50 chars
  });
  ...
  if (post.videoSrc) {
    console.log('No thumbnail found, creating placeholder');
